...
$directory = '../oc-content/themes/'.get_bloginfo('theme').'/'.$_GET['dir'].'/';

if (empty($theme)) {
    $fn = $directory . 'style.css';
    $ntemplate = 'Stylesheet';
    $ftemplate = '/'.$_GET['dir'].'/style.css';
} else {
...
function dir_nav() {
  ...
  foreach($directories as $entry) {
    if(is_file($dir_path.$entry)) {
      echo "<li><a href='?dir=".$_GET["dir"]."&theme=".$entry."'>".$entry."</a><br /><small><em>(".$entry.")</em></small></li>";
    }
  }
  echo "</ul>";
}
